...
def env_factory(**kwargs):
    ...
    if 'cassiefootstepv0' in path.lower():
        from envs.envs import CassieLocomotionStepV0Env
        return partial(CassieLocomotionStepV0Env, **kwargs)

    if 'cassiefootstepv1' in path.lower():
        from envs.envs import CassieLocomotionFootstepV1Env
        return partial(CassieLocomotionFootstepV1Env, **kwargs)

    if 'cassie_recon' in path.lower():
        from envs.envs import CassieLocomotionReconEnv
        return partial(CassieLocomotionReconEnv, **kwargs)

    if 'cassiemap' in path.lower():
        from envs.envs import CassieLocomotionMapEnv
        return partial(CassieLocomotionMapEnv, **kwargs)

    if 'cassiedepth' in path.lower():
        from envs.envs import CassieLocomotionDepthEnv
        return partial(CassieLocomotionDepthEnv, **kwargs)
    ...
    with torch.no_grad():
        ...
        while total_t < min_timesteps:
            ...
            while not done and timesteps < max_traj_len:
                if noise is None:
                    action = policy.forward(state, update_norm=update_norm, deterministic=False).numpy()
                else:
                    action = policy.forward(state, update_norm=update_norm).numpy() + np.random.normal(0, noise, size=policy.action_dim)
